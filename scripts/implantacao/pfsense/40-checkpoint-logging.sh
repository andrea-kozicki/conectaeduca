...
has_syslogd_required_forwarding() {
 ...
 BEGIN {
   ctx=""
   all=system=firewall=dns_group=dns_unbound=auth=gateway=0
 }
 {
   line=$0
   sub(/^[[:space:]]*/, "", line)
   ...
   if (ctx == "!*" && selector ~ /^\*\.[^;[:space:]]+$/) all=1
   if (ctx ~ /^!filterlog([,[:space:]]|$)/) firewall=1
   if (ctx ~ /^!dpinger([,[:space:]]|$)/) gateway=1
   if (ctx ~ /^!unbound([,[:space:]]|$)/) dns_unbound=1
   if (ctx ~ /^!dnsmasq,named,filterdns([,[:space:]]|$)/) dns_group=1
   if (selector ~ /(^|;)auth\./ && selector ~ /(^|;)authpriv\./) auth=1
   if (ctx ~ /^!-/ &&
       selector ~ /(^|;)kern\./ &&
       selector ~ /(^|;)security\./ &&
       selector ~ /(^|;)daemon\./) system=1
 }
 END {
   if (all || (system && firewall && dns_group && dns_unbound && auth && gateway))
     exit 0
   exit 1
 }
 ' "$@"
...
EOF
 ...
 printf '%s\n' "$REQUIRED_SAMPLE" | grep -v '^!dpinger [ "$(extract_f_config '/usr/sbin/syslogd -c -f /var/etc/syslog.conf')" = "/var/etc/syslog.conf" ] || {
...
exit "$RC" | has_syslogd_required_forwarding && {
