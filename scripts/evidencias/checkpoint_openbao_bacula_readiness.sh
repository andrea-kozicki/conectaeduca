...
if [[ -n "$GIT_TOP_REAL" && "$GIT_TOP_REAL" == "$ROOT_REAL" ]]; then
    ...
    SOURCE_MODE="git"
elif [[ -f "$ROOT/RELEASE-METADATA.txt" ]] && grep -Eq '^git_commit=[0-9a-f]{40}$' "$ROOT/RELEASE-METADATA.txt" && grep -Fxq 'runtime_secrets_included=no' "$ROOT/RELEASE-METADATA.txt"; then
