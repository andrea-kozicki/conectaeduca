...
elif [[ -f "$ROOT/RELEASE-METADATA.txt" ]] && grep -Eq '^git_commit=[0-9a-f]{40}$' "$ROOT/RELEASE-METADATA.txt"; then
    SOURCE_MODE="handoff"
    echo "source=handoff"
...
if [[ "$SOURCE_MODE" == "git" ]]; then
    ...
    if [[ -f "$ROOT/RELEASE-METADATA.txt" ]] && grep -Eq '^git_commit=[0-9a-f]{40}$' "$ROOT/RELEASE-METADATA.txt"; then
