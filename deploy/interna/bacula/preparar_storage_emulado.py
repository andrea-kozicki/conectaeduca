...
def run(argv: list[str], *, input_text: str | None = None, timeout: int = 120,
        show: bool = True, check: bool = False) -> tuple[int, str]:
    emit(f"\n$ {qcmd(argv)}")
    try:
        p = subprocess.run(
        ...
        if show and out: emit(out.rstrip())
        emit(f"RC=TIMEOUT({timeout}s)")
        if check: raise RuntimeError(f"timeout: {qcmd(argv)}")
        return 124, out
    out = p.stdout or ""
    if show and out: emit(out.rstrip())
    elif not show: emit(f"OUTPUT_OMITIDO={len(out.encode('utf-8'))}_bytes")
...
def inspect(name: str) -> dict[str, Any]:
    ...
    obj = json.loads(out)
    if len(obj) != 1: raise RuntimeError(f"docker inspect inesperado: {name}")
    return obj[0]


def backup_mount(ins: dict[str, Any]) -> dict[str, Any] | None:
    for m in ins.get("Mounts", []) or []:
        if m.get("Destination") == "/backup": return m
    return None
...
def compose(base: Path, files: list[Path], overlay: bool, *tail: str) -> list[str]:
    ...
    for f in files: argv += ["-f", str(f)]
    if overlay: argv += ["-f", str(base / "compose.storage-emulado.yml")]
    argv += list(tail)
    return argv
...
def fingerprint(path: Path) -> dict[str, Any]:
    rc, out = sudo(["python3", "-", str(path)], input_text=TREE_HELPER, show=False, check=True, timeout=300)
    lines = [x for x in out.splitlines() if x.strip().startswith("{")]
    if not lines: raise RuntimeError(f"fingerprint sem JSON: {path}")
...
def output_no_jobs(out: str) -> bool:
    if re.search(r"No Jobs running\.", out, re.I): return True
    m = re.search(r"Running Jobs:\s*(.*?)\n={3,}", out, re.I | re.S)
    return bool(m and (not m.group(1).strip() or re.search(r"No Jobs running", m.group(1), re.I)))


def query_director(base: Path, files: list[Path], show: bool = True) -> tuple[int, str]:
    rc, _ = run(["docker", "exec", DIRECTOR, "sh", "-lc",
                 "test -r /etc/bacula-runtime/bconsole.conf && command -v bconsole >/dev/null"], show=False)
    if rc == 0:
        return run(["docker", "exec", "-i", DIRECTOR, "bconsole", "-c", "/etc/bacula-runtime/bconsole.conf"],
                   input_text="status director\nquit\n", show=show, timeout=60)
    return sudo(compose(base, files, False, "--profile", "tools", "run", "--rm", "--no-deps", "-T", "bconsole"),
                input_text="status director\nquit\n", show=show, timeout=90)
...
def wait_running(name: str, timeout: int = READY_TIMEOUT) -> dict[str, Any]:
    ...
    while time.monotonic() < deadline:
        try:
            ins = inspect(name); st = ins.get("State") or {}; last = json.dumps(st)
            health = (st.get("Health") or {}).get("Status")
            if st.get("Running") and health in (None, "healthy"): return ins
            if st.get("Status") in ("dead", "exited"): break
...
def find_legacy_source(storage_ins: dict[str, Any]) -> tuple[str | None, Path | None, str]:
    ...
    if not m: raise RuntimeError("mount /backup ausente no Storage")
    typ = str(m.get("Type") or "")
    if typ == "volume":
        name = str(m.get("Name") or "")
        src = Path(str(m.get("Source") or ""))
        if not name or not src.is_absolute(): raise RuntimeError("named volume /backup sem Name/Source válidos")
...
def execute(mode: str, base: Path, target: Path) -> int:
    ...
    if not overlay.is_file(): raise RuntimeError(f"overlay ausente: {overlay}")
    ...
    for cmd in ("docker", "sudo", "python3"):
        ...
        if rc != 0: raise RuntimeError(f"comando ausente: {cmd}")
    if os.geteuid() == 0: raise RuntimeError("execute como usuário comum; sudo será pontual")
    ...
    if not (storage.get("State") or {}).get("Running"):
        raise RuntimeError("Storage precisa estar running no início para identificar o mount ativo")
    volume_name, source, kind = find_legacy_source(storage)
    emit(f"ACTIVE_BACKUP_KIND={kind}")
    emit(f"ACTIVE_BACKUP_SOURCE={source}")
    if volume_name: emit(f"LEGACY_VOLUME_NAME={volume_name}")

    if kind == "bind":
        if source != target:
            raise RuntimeError(f"Storage já usa bind diferente do TARGET solicitado: {source}")
    ...
    if src_fp_live.get("files", 0) == 0:
        mark("WARN", "Named volume legado não contém arquivos regulares; a migração continuará somente se solicitada.")
    ...
    director_stopped = storage_stopped = False
    try:
        run(["docker", "stop", "-t", str(STOP_TIMEOUT), DIRECTOR], timeout=STOP_TIMEOUT+20, check=True)
        director_stopped = True; mark("PASS", "Director parado; novos jobs bloqueados.")
        run(["docker", "stop", "-t", str(STOP_TIMEOUT), STORAGE], timeout=STOP_TIMEOUT+20, check=True)
        storage_stopped = True; mark("PASS", "Storage parado; mídia legado quiescente.")
        ...
        if m.get("Type") != "bind" or Path(str(m.get("Source") or "")) != target or m.get("RW") is False:
            raise RuntimeError(f"mount live pós-ativação inválido: {m}")
        mark("PASS", "Storage live usa o bind esperado em /backup.")

        post_fp = fingerprint(target)
        if post_fp != tgt_fp: raise RuntimeError("fingerprint do TARGET mudou durante ativação")
        ...
        if same_fs: mark("WARN", "Storage está no mesmo filesystem de '/': sem isolamento físico/disaster recovery.")
        else: mark("INFO", "TARGET está em filesystem distinto de '/'; ainda exige política externa de DR.")
        ...
        with contextlib.suppress(Exception):
            ...
            if (ins.get("State") or {}).get("Running"):
                run(["docker", "stop", "-t", str(STOP_TIMEOUT), DIRECTOR], timeout=STOP_TIMEOUT+20)
            director_stopped = True
        try:
            argv = compose(base, files, False, "up", "-d", "--no-deps", "--force-recreate", "storage")
            sudo(argv, timeout=240, check=True)
            storage_stopped = False
            sins = wait_running(STORAGE)
            m = backup_mount(sins) or {}
            if m.get("Type") != "volume": raise RuntimeError(f"rollback não restaurou volume: {m}")
...
def main() -> int:
    ...
    ap.add_argument("--target", default=os.environ.get("CONECTAEDUCA_BACULA_STORAGE_PATH", DEFAULT_TARGET))
    ap.add_argument("--bacula-dir", default=str(Path(__file__).resolve().parent))
    ap.add_argument("--evidence-dir", default=os.environ.get("CONECTAEDUCA_EVIDENCE_DIR", "/var/tmp/conectaeduca-evidencias"))
    args = ap.parse_args()

    base = Path(args.bacula_dir).resolve(); target = Path(args.target).resolve()
    outdir = Path(args.evidence_dir).expanduser(); outdir.mkdir(parents=True, exist_ok=True)
    ...
    class Tee(io.TextIOBase):
        ...
        def flush(self) -> None: sys.__stdout__.flush(); buf.flush()
