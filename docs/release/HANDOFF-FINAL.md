...
O workflow `.github/workflows/infra-script-tests.yml` executa `sh -n` e os self-tests do checkpoint/coletor pfSense, além de `python3 -m py_compile` no helper de Storage Bacula. O objetivo é impedir que corrupção sintática ou regressões dos casos negativos do parser voltem a passar apenas pelos gates SAST/secret scanning.
