...
| Ferret | ✅ VALIDADO no runtime | usuário `ferret`; rootfs RO; `cap_drop=ALL`; NNP; PIDs 128; tmpfs restritivo; config/inbox RO; API só em loopback; limites efetivos de 1280 MiB e 2 CPUs; health funcional monitorado pelo host via helper + timer systemd | sem pendência crítica pré-freeze; quarentena permanece evolução futura e o modo atual continua detect-only |
